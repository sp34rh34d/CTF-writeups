...
then I have used [DnSpy](https://github.com/dnSpy/dnSpy) to recover the source code from exe. here we can see the following functions
...
the function ```Main()``` calls another function ```main()``` and ```load()```, the function ```main()``` execute a ```ping``` command and then try to delete all the files intos path ```/```.
...
the parameters we can see here are  ```15b279d8c0fdbd7d4a8eea255876a0fd189f4fafd4f4124dafae47cb20a447308e3f77995d3c``` and ```73de18bfbb99db4f7cbed3156d40959e7aac7d96b29071759c9b70fb18947000be5d41ab6c41``` then call another function ```otc()``` this receive those parameters, and we see the following.
```
uint8[] otp (
			uint8[] data,
			uint8[] key
...
I write the following python code to recover the flag.
